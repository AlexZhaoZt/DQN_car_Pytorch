...
def huber_loss(y, y_hat, delta):
    ...
    less_than = 0.5 * (error ** 2)
    more_than = (delta * abs_error) - half_delta_squared

    if abs_error < delta:
        loss_per_sample = less_than
    else:
        loss_per_sample = more_than
...
class Q_net(torch.nn.Module):
    ...
    def __init__(self, inputs, outputs):
        ...
        in_channels = inputs[0]
        w = inputs[1]
        h = inputs[2]

        self.conv1 = torch.nn.Conv2d(in_channels, 16, kernel_size=8, stride=4)
...
class DeepQAgent(object):
    ...
    def __init__(self, input_shape, nb_actions,
        ...
        self.q_net = Q_net(inputs=input_shape, outputs=nb_actions)
    ...
    def train(self):
        ...
        if agent_step >= self._train_after:
            if (agent_step % self._train_interval) == 0:
                ...
                with torch.no_grad():
                    q_target = rewards + gamma * (1 - terminals) * torch.max(self.target_q_net(post_states), dim=1) #TODO: max

                q_vals = self.target_q_net(pre_states)[:, actions]

                loss = huber_loss(q_targets, q_vals, 1.0)
                ...
                if (agent_step % self._target_update_interval) == 0:
                    soft_update(self.target_q_net, self.q_net, 1)
                    torch.save(
                        self.q_net.state_dict(),
                        "models/model%d.pkl" % agent_step
                    )
